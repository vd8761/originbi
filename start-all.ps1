...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$PSScriptRoot/backend/admin-service'; npm run start:dev"
Write-Host "Launched Admin Service..."

# 2. Auth Service (Port 4002)
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$PSScriptRoot/backend/auth-service'; npm run start:dev"
Write-Host "Launched Auth Service..."

# 3. Corporate Service (Port 4003)
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$PSScriptRoot/backend/corporate-service'; npm run start:dev"
Write-Host "Launched Corporate Service..." 

# 4. Student Service (Port 4004)
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$PSScriptRoot/backend/student-service'; npm run start:dev"
Write-Host "Launched Student Service..."

# 5. Exam Engine (Go) (Port 4005)
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$PSScriptRoot/backend/exam-engine'; go run cmd/api/main.go"
