...
const JobItem: React.FC<JobItemProps> = ({
    ...
    <div className="bg-white dark:bg-white/5 rounded-2xl p-4 lg:p-[1vw] flex flex-col h-full border border-gray-100 dark:border-white/5 hover:border-brand-green/30 transition-all duration-300 group cursor-pointer shadow-[0_2px_10px_rgba(0,0,0,0.02)] hover:shadow-lg">
        <div className="flex justify-between items-start mb-4">
            <div className="flex items-center space-x-3">
                <div className="w-10 h-10 lg:w-[2.5vw] lg:h-[2.5vw] bg-gray-50 dark:bg-white rounded-full flex items-center justify-center p-1.5 overflow-hidden shadow-inner">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[#19211C]/60 dark:text-white/60 text-[10px] lg:text-[0.729vw] mt-0.5 truncate">{company} | {location}</p>
                </div>
            </div>
            <div className="text-right">
                <p className="text-[10px] lg:text-[0.729vw] font-medium text-[#19211C] dark:text-white">
                    Role Alignment : <span className="text-brand-green font-bold text-xs lg:text-[0.833vw]">{alignment}%</span>
                </p>
            </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mt-auto">
            {skills.map((skill, idx) => (
                <span key={idx} className="px-2.5 py-1 rounded-full bg-gray-50 dark:bg-white/10 border border-gray-100 dark:border-white/5 text-[#19211C]/80 dark:text-white font-medium text-[9px] lg:text-[0.625vw]">
                    {skill}
                </span>
            ))}
