...
import {BulkUploadIcon,PlusIcon,ChevronDownIcon, ArrowLeftWithoutLineIcon, ArrowRightWithoutLineIcon } from '@/components/icons';
...
const RegistrationManagement: React.FC = () => {
  ...
  const [tabCounts, setTabCounts] = useState<{ registrations: number | null; assigned: number | null }>({
      registrations: null,
      assigned: null
  });
  ...
  useEffect(() => {
      const fetchInitialCounts = async () => {
          try {
              // Parallel fetch to get counts for both tabs
              const [regRes, assignRes] = await Promise.all([
                  registrationService.getUsers({ page: 1, limit: 1, tab: 'registrations', search: '' }),
                  registrationService.getUsers({ page: 1, limit: 1, tab: 'assigned', search: '' })
              ]);
              setTabCounts({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Update the count for the active tab specifically to keep it in sync with search results
      setTabCounts(prev => ({
          ...prev,
  ...
  }, [currentPage, entriesPerPage, activeTab, debouncedSearchTerm, startDate, endDate]);
    ...
    <div className="flex flex-col h-full w-full gap-6 font-sans">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-end xl:items-center border-b border-brand-light-tertiary dark:border-brand-dark-tertiary pb-0 gap-4 xl:gap-0">
        <div className="flex items-center w-full xl:w-auto overflow-x-auto scrollbar-hide">
            ...
            >
                Registrations ({tabCounts.registrations !== null ? tabCounts.registrations : '...'})
            </button>
            <button 
            ...
            >
                Assign Assessment ({tabCounts.assigned !== null ? tabCounts.assigned : '...'})
            </button>
        </div>
      ...
      <div className="flex flex-col xl:flex-row justify-between gap-4 items-start xl:items-center">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
          ...
          <button 
            onClick={() => setView('add')}
            className="flex items-center gap-2 px-4 py-2.5 bg-brand-green rounded-lg text-sm font-medium text-white hover:bg-brand-green/90 transition-opacity shadow-lg shadow-brand-green/20"
          >
            <span>Add New</span>
