...
import {PlusIcon, ChevronDownIcon, ArrowLeftWithoutLineIcon, ArrowRightWithoutLineIcon} from '@/components/icons';
...
const ProgramsManagement: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        setError(null);
        try {
            const response = await programService.getProgramsList(currentPage, entriesPerPage, debouncedSearchTerm);
    ...
    const handleToggleStatus = async (id: string, currentStatus: boolean) => {
        try {
            ...
            setPrograms(prev => prev.map(p => p.id === id ? { ...p, status: !currentStatus } : p));
        } catch (err) {
            console.error(err);
        }
    };
        ...
        <div className="flex flex-col h-full w-full gap-6 font-sans">
            ...
            <div>
                <div className="flex items-center text-sm text-brand-text-light-secondary dark:text-brand-text-secondary mb-1">
                    ...
                    <span className="text-brand-green font-medium">Programs</span>
                </div>
                <h1 className="text-2xl sm:text-3xl font-bold text-brand-text-light-primary dark:text-white">Programs Management</h1>
            </div>
                 ...
                 <div className="relative">
                    ...
                    {showEntriesDropdown && (
                        <div className="absolute top-full right-0 mt-1 w-20 bg-brand-light-secondary dark:bg-[#303438] border border-brand-light-tertiary dark:border-white/10 rounded-lg shadow-xl z-50 overflow-hidden">
                             {[10, 25, 50].map(num => (
                                <button key={num} onClick={() => { setEntriesPerPage(num); setShowEntriesDropdown(false); setCurrentPage(1); }} className="w-full text-center py-1.5 text-sm hover:bg-black/5 dark:hover:bg-white/10 text-brand-text-light-primary dark:text-white">{num}</button>
                             ))}
                        </div>
                    )}
                 </div>
                 <span className="text-sm text-brand-text-light-secondary dark:text-brand-text-secondary whitespace-nowrap">of {totalCount} entries</span>
                 
                 <div className="flex items-center gap-2 ml-2">
                     <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="w-8 h-8 rounded-full bg-brand-light-tertiary dark:bg-[#303438] flex items-center justify-center text-gray-400 hover:text-white disabled:opacity-50"><ArrowLeftWithoutLineIcon className="w-3 h-3" /></button>
                     <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="w-8 h-8 rounded-full bg-brand-green flex items-center justify-center text-white shadow-lg disabled:opacity-50"><ArrowRightWithoutLineIcon className="w-3 h-3" /></button>
                 </div>
            </div>
