...
const BulkUploadRegistration: React.FC<BulkUploadRegistrationProps> = ({ onCancel }) => {
    ...
    useEffect(() => {
        ...
        registrationService.getGroups().then(setGroups).catch(console.error);
    }, []);
    ...
    useEffect(() => {
        ...
        if (view === 'processing' && importId) {
            interval = setInterval(async () => {
                try {
                    const status = await registrationService.getBulkJobStatus(importId);
                    if (status.status === 'COMPLETED') {
                        if (status.failed > 0) {
                            // Job finished with errors. Fetch updated rows to show them in Review table.
                            const rows = await registrationService.getBulkJobRows(importId);
                            ...
                            setView('review');
                            // Optionally set an error banner if the UI supports it in 'review' mode, 
                            // but the table rows will be red due to 'FAILED' status + errorMessage logic.
                        } else {
                            setSummary({
                                total: status.total,
    ...
    const handleFileSelected = async (file: File) => {
        ...
        try {
            const data = await registrationService.bulkPreview(file);
        ...
        <div className="p-6 bg-[#0B0D0F] min-h-screen">
            ...
            {view === 'success' && (
                <BulkSuccessSummary
                    ...
                    onViewAll={() => window.location.href = '/admin/registrations'} // Or use router
                />
            )}
        </div>
    );
