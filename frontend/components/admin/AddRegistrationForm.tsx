...
import { ProgramData, Department } from '@/lib/types';
...
const AddRegistrationForm: React.FC<AddRegistrationFormProps> = ({ onCancel, onRegister }) => {
    ...
    const [formData, setFormData] = useState<CreateRegistrationDto>({
        ...
        countryCode: '+91',
        mobile: '',
        programType: '',     // stores program.id (string)
        groupName: '',
        sendEmail: false,
        examStart: '',
        examEnd: '',
        ...
        const fetchInitialData = async () => {
            try {
                const [progRes, deptRes] = await Promise.all([
                    registrationService.getPrograms(),    // hits /admin/programs
                ...
                // - map code/name if backend uses different keys
                const rawPrograms: any[] = Array.isArray(progRes)
                    ? progRes
                    : (progRes as any)?.data || [];

                const normalizedPrograms: ProgramData[] = rawPrograms.map((p: any) => ({
                    id: String(p.id),
                ...
                setError("Failed to load programs or departments. Please try again.");
            }
        };
        fetchInitialData();
    }, []);

    // --- Helpers for program code behaviour ---
    // support both old ('SCHOOL', 'COLLEGE') and new ('SCHOOL_STUDENT', 'COLLEGE_STUDENT') codes
    const isSchoolProgram = (programCode?: string) =>
        programCode === 'SCHOOL' || programCode === 'SCHOOL_STUDENT';

    const isCollegeProgram = (programCode?: string) =>
        programCode === 'COLLEGE' || programCode === 'COLLEGE_STUDENT';
    ...
    const validateForm = () => {
        ...
        // SCHOOL logic
        if (isSchoolProgram(programCode)) {
        ...
        // COLLEGE logic
        if (isCollegeProgram(programCode)) {
    ...
    // --- Derived values based on selected program type ---
    const selectedProgram = programs.find(p => p.id === formData.programType);
    const currentProgramCode = selectedProgram?.programCode;

    // Only show active programs (STATUS = ON)
    const programOptions = programs
        .filter(p => p.status)
        .map(p => ({
            value: p.id,            // always string now
            label: p.programName,
        }));
        ...
        <div className="w-full font-sans animate-fade-in pb-12">
            ...
            <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-4 mb-8">
                <div>
                    ...
                    <h1 className="text-2xl sm:text-3xl font-bold text-brand-text-light-primary dark:text-white tracking-tight">
                        Add Registrations
                    </h1>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-[#1A1D21] border border-gray-200 dark:border-[#24272B] rounded-3xl p-5 sm:p-8 shadow-sm dark:shadow-xl transition-colors duration-300 relative">
                ...
                <div className="mb-8">
                    <h2 className="text-base font-bold text-brand-text-light-primary dark:text-white mb-6">
                        Basic Information
                    </h2>
                            ...
                            <div className="flex bg-gray-200 dark:bg-[#303438] rounded-full p-1 h-[50px]">
                                {['Male', 'Female', 'Other'].map(gender => (
                                    <button
                                        ...
                                        className={`flex-1 text-sm font-bold rounded-full transition-all duration-300 cursor-pointer ${formData.gender === gender
                ...
                <div className="mb-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
                        ...
                        {isSchoolProgram(currentProgramCode) && (
                            <div
                        ...
                        {isSchoolProgram(currentProgramCode) && formData.schoolLevel === 'HSC' && (
                            <div
                        ...
                        {isCollegeProgram(currentProgramCode) && (
                            <div
                        ...
                        {(
                            ...
                            isCollegeProgram(currentProgramCode)
                                ...
                                <div className="space-y-2 animate-fade-in relative z-0">
                                    ...
                                    <input
                                        type="text"
                                        maxLength={isSchoolProgram(currentProgramCode) ? 2 : 4}
                                        ...
                                        placeholder={isSchoolProgram(currentProgramCode) ? "12" : "2025"}
                        ...
                        <div className="space-y-2 relative z-0">
                            ...
                            <div className="flex bg-gray-200 dark:bg-[#303438] rounded-full p-1 w-full h-[50px]">
                                <button
                                    ...
                                    className={`flex-1 text-sm font-bold rounded-full transition-all duration-300 cursor-pointer ${formData.sendEmail
                                        ? 'bg-brand-green text-white shadow-lg shadow-green-900/20'
                                        : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-white'
                                        }`}
                                >
