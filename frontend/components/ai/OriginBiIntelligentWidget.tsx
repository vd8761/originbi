...
export default function OriginBiIntelligentWidget() {
  ...
  // Only show on corporate pages (not on the ask-ai page itself)
  const isCorporate = pathname?.includes('/corporate');
  const isAskAIPage = pathname?.includes('/corporate/ask-ai');

  if (!isCorporate || isAskAIPage) return null;
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <span className="absolute inset-0 rounded-full animate-ping bg-brand-green/30 pointer-events-none" />
