...
export class SettingsService {
    ...
    async getEmailConfig(overrideKey: string): Promise<{
        ...
        const globalBccAddresses = await this.getValue<string[]>('email', 'bcc_addresses') || [];
        const globalReplyTo = await this.getValue<string>('email', 'reply_to_address') || '';
        ...
        if (localConfig && localConfig.mode === 'local') {
            return {
                ...
                bccAddresses: Array.isArray(localConfig.bcc_addresses) ? localConfig.bcc_addresses : [],
