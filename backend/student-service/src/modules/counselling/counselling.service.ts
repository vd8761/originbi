...
export class CounsellingService {
  ...
  async handleEnquiry(payload: any): Promise<any> {
    ...
    if (
      typeof willing === 'string' &&
      (willing.toLowerCase().includes('yes') ||
        willing.toLowerCase() === 'true')
    )
      isWilling = true;
    ...
    if (!mobile) {
      throw new BadRequestException(
        'Mobile number is required in contact_information',
      );
    }

    // Check for duplicate registration (Mobile OR Email)
    const duplicateCheckQuery: any[] = [{ mobileNumber: mobile.toString() }];
    ...
    if (existingSession) {
      ...
      if (existingSession.mobileNumber === mobile.toString())
    ...
    if (payload.counselling_type_id) {
      ...
      if (!selectedType) {
        throw new BadRequestException(
          `Invalid or inactive Counselling Type ID: ${payload.counselling_type_id}`,
        );
      }
    } else {
    ...
    if (payload.corporate_id) {
      ...
      if (corporate.availableCredits < 1) {
        throw new BadRequestException({
          message:
            'Insufficient credits in corporate account. Please contact your administrator.',
          statusCode: 400,
    ...
    const triggerUrl = this.configService.get<string>(
      'COUNSELLING_TRIGGER_URL',
    );
    if (triggerUrl) {
      try {
        ...
        this.httpService
          ...
          .subscribe({
            ...
            error: (err) =>
              console.error(`Failed to trigger external API: ${err.message}`),
          });
      } catch (error) {
  ...
  async verifySessionAccess(
    ...
    // Try matching email
    if (!match && email && identifier.toLowerCase() === email.toLowerCase())
      match = true;

    if (!match) {
      throw new BadRequestException(
        'Email or Mobile Number does not match our records.',
      );
    }
  ...
  async completeSession(sessionId: number): Promise<any> {
    ...
    const sortedTraits = Object.entries(scores).sort((a, b) => {
    ...
    this.triggerReportGeneration(session.id).catch((err) => {
      console.error(
        `Failed to trigger report generation for session ${session.id}:`,
        err.message,
      );
    });
