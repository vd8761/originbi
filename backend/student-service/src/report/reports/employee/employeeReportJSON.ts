 ...
 * ---------------------
 * Standalone JSON builder that mirrors the computation logic of EmployeeReport
 * but returns structured JSON instead of generating a PDF.
 *
 * Usage:
 *   const json = await buildEmployeeReportJSON(employeeData);
 *   res.json(json);
 */
...
function buildWordSketch(data: EmployeeData) {
  ...
  return {
    ...
    staticRows,
    introText: {
      paragraph1: EMPLOYEE_CONTENT.natural_style_work_sketch_desc,
      paragraph2: EMPLOYEE_CONTENT.natural_style_work_sketch_desc_1,
    },
  };
}
...
export async function buildEmployeeReportJSON(
  ...
  const response = {
    meta: {
      ...
      reportType: 'employee',
      tableOfContents: EMPLOYEE_TOC_CONTENT.map((item) =>
    ...
    sections: {
      // ── About the Report ──
      aboutReport: {
        title: EMPLOYEE_TOC_CONTENT[0],
        content: EMPLOYEE_CONTENT.about_report,
        purpose: EMPLOYEE_CONTENT.purpose_items,
        whyMatters: {
          intro: EMPLOYEE_CONTENT.why_matters,
          items: EMPLOYEE_CONTENT.why_matters_items,
        },
        whatYouGain: EMPLOYEE_CONTENT.what_you_gain,
        aboutSelfDiscovery: EMPLOYEE_CONTENT.about_obi_self_discovery_report,
      },

      // ── Career Path Benefits ──
      careerPathBenefits: {
        title: EMPLOYEE_TOC_CONTENT[1],
        benefitsIntro: EMPLOYEE_CONTENT.benefits_identifying_career_paths,
        benefitsPara2:
          EMPLOYEE_CONTENT.benefits_identifying_career_paths_para_2,
        whyRightCareerMatters:
          EMPLOYEE_CONTENT.why_identifying_right_career_matters,
        whyRightCareerMattersPara2:
          EMPLOYEE_CONTENT.why_identifying_right_career_matters_para_2,
        howThisReportHelps: EMPLOYEE_CONTENT.how_this_report_helps_you,
        howThisReportHelpsList: EMPLOYEE_CONTENT.how_this_report_helps_list,
        howThisReportHelpsPara2:
          EMPLOYEE_CONTENT.how_this_report_helps_you_para_2,
        importantNote: EMPLOYEE_CONTENT.important_note,
      },
  ...
  logger.info('[EmployeeReportJSON] JSON report built successfully.');
  return response;
}
