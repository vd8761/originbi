...
export class CollegeReport extends BaseReport {
  ...
  private generateContent1(): void {
    ...
    this.pHtml(contentBlock.your_strength_what_you_bring_to_the_organization_1);
    this.h2('Your Natural Strengths');
    this.list(contentBlock.your_strength_what_you_bring_to_the_organization_2, {
      indent: 30,
    });
    ...
    this.pHtml(contentBlock.motivations_and_need_your_personalized_insights_2);
    this.ensureSpace(0.1, true);
    this.pHtml(
    ...
    this.h3(contentBlock.your_personalized_behavioral_understanding_the_graphs);
    this.list(
          ...
          if (typeof section.content === 'string') {
            ...
            section.content.forEach((item: any) => {
              ...
              if (item.subtitle && item.text) {
                this.list([`<b>${item.subtitle} :</b> ${item.text}`], {
                  indent: 20,
                  gap: 0,
                });
              } else {
  ...
  private renderTipBox(text: string): void {
    ...
    this.doc.save();
    this.doc.rect(startX, startY, width, boxHeight).fillColor('#F0F8FF').fill();
    this.doc
