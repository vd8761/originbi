...
export class BaseReport {
  ...
  private registerFonts(): void {
    const fonts = [
      { id: 'Sora-Bold', path: 'public/assets/fonts/Sora-Bold.ttf' },
      { id: 'Sora-SemiBold', path: 'public/assets/fonts/Sora-SemiBold.ttf' },
      { id: 'Sora-Regular', path: 'public/assets/fonts/Sora-Regular.ttf' },
      { id: 'Inter-Regular', path: 'public/assets/fonts/Inter-Regular.ttf' },
      { id: 'Inter-Bold', path: 'public/assets/fonts/Inter-Bold.ttf' },
      { id: 'Inter-SemiBold', path: 'public/assets/fonts/Inter-SemiBold.ttf' },
      { id: 'Inter-Italic', path: 'public/assets/fonts/Inter-Italic.ttf' },
    ];
  ...
  private setupPageListener(): void {
    this.doc.on('pageAdded', () => {
      if (this._currentBackground && fs.existsSync(this._currentBackground)) {
  ...
  protected ensureSpace(
    ...
    if (usePercentage) {
      neededHeight = (this.PAGE_HEIGHT - 2 * this.MARGIN_STD) * requiredSpace;
    }
  ...
  protected list(items: string[], opts: TextOptions = {}) {
    ...
    if (type === 'bullet') {
      items.forEach((item) => {
        ...
        this.doc
          .circle(currentX + 2.5, itemY + (opts.fontSize || 12) / 2 - 1, 2.5)
          .fill();
  ...
  protected Image(path: string, opts: ImageOptions = {}) {
    if (!fs.existsSync(path)) {
      console.warn(`[Report service]`, `Image missing: ${path}`);
      return;
    }
    ...
    data.forEach((item, index) => {
      ...
      this.doc
        ...
        .polygon(
          ...
          [currentX + barWidth - sideViewWidth, startY + maxBarHeight + 10],
          [
      ...
      this.doc
        ...
        .text(
          ...
          currentX + (barWidth - textBoxWidth) / 2 + percentageLabelOffset,
      ...
      this.doc
        ...
        .text(
          item.label,
          currentX + barWidth - sideViewWidth + 12 + categoryLabelOffset,
  ...
  protected drawSingleBarChart(data: any[], options: any = {}) {
    ...
    const startX =
      (this.PAGE_WIDTH -
        (data.length * config.barWidth + (data.length - 1) * config.barGap) +
        config.slantDepth) /
      2;
  ...
  protected drawSideBySideBarCharts(
    ...
    const singleGraphWidth =
      dataLeft.length * config.barWidth + (dataLeft.length - 1) * config.barGap;
    const leftChartX =
  ...
  protected table(
    ...
    const drawCellRect = (
      ...
      if (!fill || fill === 'transparent') {
        this.doc.lineWidth(borderWidth).rect(xx, yy, w, h).stroke(stroke);
      } else {
        this.doc
    ...
    if (fillCount > 0) {
      ...
      definedWidths.forEach((w, i) => {
        if (w === 'fill' || w === undefined) finalColWidths[i] = fillWidth;
      });
    }
    ...
    const finalHeaderHeight =
      options.headerHeight || Math.max(maxHeaderHeight + 2 * cellPadding, 20);
    ...
    if (options.mergeRepeatedHeaders) {
      ...
      spans.forEach((spanItem) => {
        ...
        this.doc
          ...
          .text(spanItem.text, currentX + cellPadding, currentY + yOffset, {
            width: mergedWidth - 2 * cellPadding,
    ...
    rows.forEach((rowOrObj, rowIndex, allRows) => {
      ...
      let spans: { text: string; span: number; colIndex: number }[] | null =
        null;
      if (isSubHeader) {
      ...
      if (isSubHeader && spans) {
        ...
        const fontToUse =
          ...
          this.FONT_SORA_BOLD;
        const fontSizeToUse =
          rowStyle?.fontSize || options.headerFontSize || baseFontSize;
        ...
        const fontToUse = isStyledRow && rowStyle?.font ? rowStyle.font : null;
        const fontSizeToUse =
          isStyledRow && rowStyle?.fontSize ? rowStyle.fontSize : baseFontSize;
        ...
        rowData.forEach((val, i) => {
          ...
          if (isStyledRow && rowStyle?.mergeSupportedColumn && rowIndex > 0) {
      ...
      // B. Page Break Check
      let pageBroke = false;
      if (currentY + finalRowHeight > this.PAGE_HEIGHT - this.MARGIN_STD) {
        this.doc.addPage();
      ...
      if (isSubHeader) {
        ...
        txtColor = isAlternate
          ? options.alternateRowTextColor || options.rowTextColor || 'black'
          ...
          if (!isMergedAbove) {
            ...
            if (
              ...
              rCell.content.forEach((item) => {
                ...
                this.doc.text(item.text, currentX + cellPadding, currentTextY, {
                  width: cw - 2 * cellPadding,
                  align:
                    isStyledRow && rowStyle?.align
                      ? rowStyle.align
              ...
              this.doc
                ...
                .fontSize(fontSizeToUse)
                .fillColor(txtColor)
                .text(cellText, currentX + cellPadding, currentY + yOffset, {
                  width: cw - 2 * cellPadding,
                  align:
                    isStyledRow && rowStyle?.align
                      ? rowStyle.align
  ...
  protected roadmap(data: RoadmapItem[], options: RoadmapOptions = {}) {
    ...
    data.forEach((item, index) => {
      ...
      const totalItemHeight = Math.max(hTitle + hDate + hDesc, nodeRadius * 2);
  ...
  protected drawRadarChart(
    ...
    for (let i = 0; i < numAxes; i++) {
      ...
      const r =
        levelOneRadius + (finalRadius - levelOneRadius) * (val / maxValue);
  ...
  protected addFooters(text: string): void {
    ...
    for (let i = 1; i < totalPages; i++) {
      ...
      this.doc.fillColor('black').text(siteTitle, this.MARGIN_STD, textY, {
        lineBreak: false,
        baseline: 'top',
      });
