...
//         // We want a 2x2 grid layout. Let's get up to 4 departments max for this layout.
//         const deptEntries = Array.from(departmentMap.entries()).slice(0, 4);

//         if (deptEntries.length > 0) {
//             this.ensureSpace(0.5, true);

//             const gridMargin = this._useStdMargins ? this.MARGIN_STD : 15 * this.MM;
//             const quadWidth = (this.PAGE_WIDTH - 2 * gridMargin - 10 * this.MM) / 2; // 10mm gap between columns

//             let currentY = this.doc.y;
//             let startY = currentY;

//             // We need to keep track of the maximum Y reached in the current row
//             let maxRowY = startY;
...
//                 const currentX = isRightColumn ? gridMargin + quadWidth + 10 * this.MM : gridMargin;

//                 // Reset to start of row if it's the left column
//                 if (!isRightColumn) {
//                     startY = maxRowY; //  + (i > 0 ? 3 * this.MM : 0); // Add gap between rows if not first row
//                     currentY = startY;
//                 } else {
//                     currentY = startY; // align right column with the left
//                 }
...
//                     .fontSize(11)
//                     .fillColor(this.COLOR_DEEP_BLUE)
//                     .text(deptName, currentX, currentY, { width: quadWidth });

//                 currentY = this.doc.y + 4 * this.MM;

//                 // Render the inner chart within this quadrant bound
...
//         // For Quadrant: text goes above the bar, so the bar spans almost full width of the quadrant
//         // Decrease barHeight slightly, reduce space to fit text comfortably
...
//         // Ensure we have space on the page (skip this in quadrant mode as it breaks layout)
//         if (!isQuadrant && chartBottom + 50 * this.MM > this.PAGE_HEIGHT) {
//             this.doc.addPage();
...
//             if (isQuadrant) {
//                 const y = chartTop + index * groupHeight;
...
//                     .fontSize(7)
//                     .fillColor('#333333')
//                     .text(item.course_name, chartLeft, y, {
//                         width: maxBarWidth,
//                         align: 'left',
//                         height: textHeight,
//                     });

//                 // C. Draw Bar Rect (Below Label)
//                 const barY = y + textHeight;
//                 if (barWidth > 0) {
//                     const radius = Math.min(barHeight / 2, barWidth / 2);
//                     this.doc
//                         .roundedRect(chartLeft, barY, barWidth, barHeight, radius)
//                         .fillColor(barColor)
//                         .fill();
//                 }
...
//         // --- 6. Footer Label ---
//         let footerY = chartBottom + 8 * this.MM;
//         // if (!isQuadrant) {
//         //   this.doc
//         //     .font(this.FONT_SORA_REGULAR)
//         //     .fontSize(10)
...
//         // Move doc cursor past the footer label
//         this.doc.y = footerY + 6 * this.MM;
