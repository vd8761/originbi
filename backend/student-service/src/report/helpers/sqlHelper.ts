...
export async function testDbConnection() {
  try {
    ...
    logger.info('[DB] ✅ Successfully connected to the PostgreSQL database');
    client.release();
  } catch (error) {
    logger.error(
...
export async function getCareerGuidanceByTrait(
  ...
  try {
    ...
    const rolesResult = await client.query(rolesQuery, [traitId, departmentId]);
...
export async function updateReportPassword(
  ...
  try {
    ...
    const checkQuery = `
        ...
        `;
    const checkResult = await client.query(checkQuery, [assessmentSessionId]);
