  ...
  async withRetry<T>(
    ...
    try {
      ...
      if (retries > 0 && isRateLimit) {
        this.logger.warn(
          `Rate limit hit in CorporateService. Retrying in ${delay}ms... (${retries} retries left)`,
        );
  ...
  private async createCognitoUser(
    ...
    try {
      ...
      const res = await this.withRetry(() =>
        firstValueFrom(this.http.post(url, { email, password, groupName })),
      );
      ...
      this.logger.error(
        `Error creating Cognito user (Status: ${status}):`,
        authErr,
      );
    ...
    try {
      ...
      if (sortBy) {
        ...
        switch (sortBy) {
          ...
          case 'user.email':
            sortCol = 'u.email';
            break;
          case 'user.mobile_number':
            sortCol = 'c.mobileNumber';
            break;

          // For things we can't easily sort (like metadata json), we skip or fallback
          default:
            sortCol = 'c.createdAt'; // Fallback
        }
      ...
      return {
        data: rows.map((r) => ({
          ...
          id: r.id,
          full_name:
            (r.user?.metadata as { fullName?: string })?.fullName || '',
          email: r.user?.email || '',
