...
async function bootstrap() {
    ...
    try {
        const ExtensionResult = await dataSource.query(`SELECT * FROM pg_extension WHERE extname = 'vector'`);
    ...
    try {
        const countResult = await dataSource.query(`SELECT COUNT(*) as count FROM rag_embeddings`);
        const docCountResult = await dataSource.query(`SELECT COUNT(*) as count FROM rag_documents`);
        ...
        if (parseInt(vectorCount) > 0) {
            ...
            logger.warn('⚠️  Vector Store is EMPTY. SyncService might need time to run.');
        }
    } catch (e) {
        logger.error(`❌ Vector Count Failed: ${e.message} (Did you run the migration?)`);
    }
    ...
    try {
        ...
        if (!queryEmbedding) {
            logger.error('❌ Failed to generate embedding for query (Check Jina AI Key)');
        } else {
            ...
            const vectorSql = `
                ...
                LIMIT 3
            `;
            const results = await dataSource.query(vectorSql, [`[${queryEmbedding.join(',')}]`]);
