...
export class SyncService implements OnModuleInit, OnModuleDestroy {
    ...
    private async runSync(): Promise<any> {
        ...
        try {
            ...
            this.logger.log(`✅ Sync complete in ${duration}s. Indexed ${results.total} documents.`);
     ...
     */
    private async syncAllTables(): Promise<{ total: number; details: Record<string, number> }> {
        let total = 0;
...
Average Score: ${parseFloat(r.avg_score || 0).toFixed(1)}%`,
        ...
        for (const source of tableSources) {
            try {
                ...
                const result = await this.embeddingsService.bulkUpsertDocuments(documents);
