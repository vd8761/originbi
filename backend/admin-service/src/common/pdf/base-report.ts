...
export class BaseReport {
  ...
  private registerFonts(): void {
    ...
    fonts.forEach((font) => {
      ...
      if (fs.existsSync(fontPath)) {
        ...
        console.warn(
          `Font missing, using fallback: ${font.id} -> ${font.fallback}`,
        );
  ...
  protected ensureSpace(requiredSpace: number, usePercentage = false): void {
  ...
  protected pHtml(text: string, opts: TextOptions = {}) {
    const x = opts.x ?? (this._useStdMargins ? this.doc.x : this.MARGIN_STD);
    ...
    parts.forEach((part, index) => {
      if (part === '<b>') isBold = true;
        ...
        this.doc.font(isBold ? this.FONT_BOLD : this.FONT_REGULAR).text(part, {
          width: width,
