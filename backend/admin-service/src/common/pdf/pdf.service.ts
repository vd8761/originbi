...
import {
  OverallFitmentReport,
  OverallReportData,
} from './overall-fitment-report';
...
export class PdfService {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        report.doc.on('end', () => {
          ...
          this.logger.log(
            `📄 Generated Overall Report PDF: ${pdfData.length} bytes`,
          );
          resolve(pdfData);
        });
  ...
  async generateCareerFitmentReport(
    data: CareerFitmentReportData,
  ): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        report.doc.on('end', () => {
          const pdfData = Buffer.concat(buffers);
          this.logger.log(
            `📄 Generated Career Fitment Report PDF: ${pdfData.length} bytes`,
          );
          resolve(pdfData);
        });
        ...
        report.doc.on('error', (err: Error) => {
          this.logger.error(
            `❌ Error generating Career Fitment PDF: ${err.message}`,
            err.stack,
          );
          reject(err);
        });
        ...
        this.logger.error(
          'Failed to initiate Career Fitment PDF generation',
          error,
        );
        reject(error instanceof Error ? error : new Error(String(error)));
      }
