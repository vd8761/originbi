...
export class OverallFitmentReport extends BaseReport {
  ...
  private generateCoverPage(): void {
    ...
    this.doc
      ...
      .fillColor(this.COLOR_DEEP_BLUE)
      .text(
        `${this.data.candidates.length} Candidates Assessed`,
        this.PAGE_WIDTH - 220,
        bottomY + 40,
        {
          width: 180,
  ...
  private generateExecutiveSummary(): void {
    ...
    // Introduction Text
    this.p(
      'This report provides a comprehensive analysis of candidate readiness for future roles within the organization. The assessment evaluates behavioral alignment, skill capabilities, and transition potential.',
    );
    ...
    const highReadiness = this.data.candidates.filter(
      (c) => c.futureRoleReadinessScore >= 80,
    ).length;
    const moderateReadiness = this.data.candidates.filter(
      (c) =>
        c.futureRoleReadinessScore >= 60 && c.futureRoleReadinessScore < 80,
    ).length;
    const lowReadiness = this.data.candidates.filter(
      (c) => c.futureRoleReadinessScore < 60,
    ).length;
  ...
  // CANDIDATE PROFILE
  // ═══════════════════════════════════════════════════════════════════════════
  private generateCandidateProfile(
    ...
    this.doc
      ...
      .fillColor(this.COLOR_BLACK)
      .text(
        `Target Role: ${candidate.expectedFutureRole}`,
        this.MARGIN_STD,
        this.doc.y + 6,
      );
    ...
    const showMapping =
      candidate.currentRole !== 'Aspiring Professional' &&
      candidate.expectedFutureRole !== 'Next Level Role';
    ...
    const readinessRows = candidate.readinessDimensions.map((d) => [
      d.dimension,
      d.alignment,
    ]);
    ...
    this.p(
      `To transition from ${candidate.currentRole} to ${candidate.expectedFutureRole}, the following shifts are required:`,
    );
  ...
  private generateAppendix(): void {
    ...
    this.doc
      ...
      .fillColor(this.COLOR_BLACK)
      .text(
        "This report utilizes Origin BI's proprietary assessment methodology combining:",
        this.MARGIN_STD,
        this.doc.y,
        {
          width: this.PAGE_WIDTH - 2 * this.MARGIN_STD,
  ...
  private drawLegendTable(
    legendData: { range: string; label: string; color: string }[],
  ): void {
