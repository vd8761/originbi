...
export class UserEnrichmentService {
  ...
  async enrichFromCognito(cognitoPayload: {
    ...
    this.logger.debug(
      `Enriching user context for cognito_sub=${cognitoSub}, role=${cognitoRole}`,
    );
    ...
    if (userContext.role === 'CORPORATE') {
      ...
      if (corporateAccountId) {
        ...
        this.logger.debug(
          `Resolved corporateAccountId=${corporateAccountId} for user ${userContext.id}`,
        );
      } else {
        this.logger.warn(
    ...
    try {
      ...
      if (email) {
        ...
        if (emailRows.length > 0) {
          ...
          this.logger.log(
            `Linked cognito_sub=${cognitoSub} to user id=${emailRows[0].id}`,
          );
          return emailRows[0];
        }
      }
      ...
      this.logger.error(
        `DB lookup failed for cognito_sub=${cognitoSub}: ${error.message}`,
      );
      return null;
    }
  }
  ...
  private async findUserById(userId: number): Promise<any> {
    try {
      ...
      this.logger.error(
        `DB lookup failed for userId=${userId}: ${error.message}`,
      );
      return null;
    }
  }
  ...
  private async findCorporateAccountId(userId: number): Promise<number | null> {
    try {
      ...
      this.logger.error(
        `Corporate account lookup failed for userId=${userId}: ${error.message}`,
      );
      return null;
    }
  }
