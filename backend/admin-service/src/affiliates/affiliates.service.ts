...
export class AffiliatesService {
  ...
  async updateReadyToProcessStatus() {
    ...
    try {
      ...
      throw new InternalServerErrorException(
        `Failed to refresh ready to payment status: ${error.message}`,
      );
    }
  }
  ...
  async findAll(
    ...
    try {
      ...
      await this.updateReadyToProcessStatus().catch((err) =>
        this.logger.warn(
          `Non-critical: Failed to refresh ready-to-process statuses: ${err.message}`,
        ),
      );
      ...
      this.logger.error(
        `findAll Affiliates Error: ${error.message}`,
        error.stack,
      );
      throw new InternalServerErrorException(
        `Failed to fetch affiliates: ${error.message}`,
      );
    }
  }

  private async calculateReadyToProcessAmount(
    affiliateId: number,
    ...
    const readyToProcessRes = await this.referralTransactionRepo
      ...
      .where('t.affiliate_account_id = :affiliateAccountId', {
  ...
  async getAdminDashboardStats() {
    try {
      ...
      for (const a of affiliates) {
        const amount = await this.calculateReadyToProcessAmount(
          a.id,
          Number(a.totalSettledCommission) || 0,
        );
        if (amount > 0) {
      ...
      this.logger.error(
        `Error getting admin dashboard stats: ${error.message}`,
      );
  ...
  private async formatAffiliate(a: AffiliateAccount) {
    ...
    const readyToProcessAmount = await this.calculateReadyToProcessAmount(
      a.id,
      Number(a.totalSettledCommission) || 0,
    );
  ...
  async previewSettlement(affiliateId: number, amount: number) {
    ...
    const settledTxnSumRes = await this.referralTransactionRepo
      ...
      .getRawOne();
    const settledTxnSum = parseFloat(settledTxnSumRes?.sum) || 0;
    const currentUnpaidOverflow = Math.max(
      0,
      (Number(affiliate.totalSettledCommission) || 0) - settledTxnSum,
    );
    ...
    if (suggestedLower !== null && Math.abs(suggestedLower - amount) < 0.01)
      suggestedLower = null;
    if (suggestedUpper !== null && Math.abs(suggestedUpper - amount) < 0.01)
      suggestedUpper = null;

    return {
      fullySettledCount,
      isCleanBoundary: cumulativeBoundaries.some(
        (b) => Math.abs(b - amount) < 0.01,
      ),
      transactions: transactionBreakdown,
      suggestedLower: suggestedLower
        ? Math.round(suggestedLower * 100) / 100
        : null,
      suggestedUpper: suggestedUpper
        ? Math.round(suggestedUpper * 100) / 100
        : null,
    };
  }
  ...
  async getRecentReferrals(affiliateId: number, limit: number) {
    ...
    return transactions.map((t) => {
      ...
      const studentName =
        reg?.fullName || reg?.user?.email?.split('@')[0] || 'N/A';
      const studentEmail = reg?.user?.email || 'N/A';
      const settledDown: string =
        t.settlementStatus === 2
          ? 'Settled'
          : t.settlementStatus === 1
            ? 'Processing'
            : 'Not Settled';
      return {
        id: t.id,
  ...
  async getReferralsPage(
    ...
    if (search) {
      ...
      const [allRows, allTotal] =
        await this.referralTransactionRepo.findAndCount({
          where: finalWhere,
          relations: ['registration', 'registration.user'],
        });
    ...
    const allCount = await this.referralTransactionRepo.count({
      where: { affiliateAccountId: affiliateId },
    });
    const pendingCount =
      (await this.referralTransactionRepo.count({
        where: { affiliateAccountId: affiliateId, settlementStatus: 0 as any },
      })) +
      (await this.referralTransactionRepo.count({
        where: { affiliateAccountId: affiliateId, settlementStatus: 1 as any },
      }));
    const completedCount = await this.referralTransactionRepo.count({
      where: { affiliateAccountId: affiliateId, settlementStatus: 2 as any },
    });
  ...
  async getEarningsStats(affiliateId: number) {
    ...
    return {
      ...
      totalSettled:
        parseFloat(settledRes?.sum) ||
        Number(affiliate.totalSettledCommission) ||
        0,
  ...
  async updateProfile(
    ...
    if (dto.mobileNumber !== undefined)
      affiliate.mobileNumber = dto.mobileNumber;
  ...
  async updatePayoutSettings(
    ...
    if (dto.accountNumber !== undefined)
      affiliate.accountNumber = dto.accountNumber;
      ...
      return await this.dataSource.transaction(async (manager) => {
        ...
        const settlement = manager.create(AffiliateSettlementTransaction, {
          ...
          metadata: {
            ...
            pendingAsOfSettlement:
              Number(affiliate.totalPendingCommission) || 0,
            settledAsOfSettlement:
              Number(affiliate.totalSettledCommission) || 0,
          },
        });
        ...
        let remainingAmountForStatusUpdate =
          currentUnpaidOverflow + dto.settleAmount;
