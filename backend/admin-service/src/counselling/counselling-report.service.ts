...
export class CounsellingReportService {
  ...
  constructor(
    ...
    if (apiKey) {
      ...
      this.logger.warn(
        '⚠️ GROQ_API_KEY not found - AI reports will use fallback generation',
      );
    }
  }
  ...
  async generateReport(sessionId: number): Promise<CounsellingReportData> {
    this.logger.log(
      `📊 Generating counselling report for session: ${sessionId}`,
    );
  ...
  private async getCourseDataset(): Promise<CourseDataset> {
    try {
      ...
      this.logger.warn(
        `⚠️ Failed to fetch courses from DB, using defaults: ${error.message}`,
      );
      return this.getDefaultCourseDataset();
    }
  }
...
Now generate the JSON report.`;
  ...
  private generateFallbackReport(
    ...
    > = {
      D: {
        behavioral:
          'Shows a direct, results-oriented work style with strong leadership potential.',
        strengths: [
      ...
      I: {
        behavioral:
          'Displays an enthusiastic, people-oriented approach with excellent communication skills.',
        strengths: [
      ...
      S: {
        behavioral:
          'Shows a stable, disciplined, and responsibility-oriented work style. Prefers clear instructions, structured learning, and practical execution.',
        strengths: [
        ...
        abilities: [
          ...
          'Inspection and quality tasks',
          'Equipment handling',
          'Team coordination',
          'Steady hands-on learning',
        ],
        growth: [
      ...
      C: {
        behavioral:
          'Demonstrates analytical thinking with attention to quality and accuracy in all work.',
        strengths: [
      ...
      career_roadmap: [
        {
          ...
          bullets: [
            ...
            'Begin work as Technician / Inspector',
          ],
        },
        {
          stage: 'Year 1–3',
          bullets: [
            ...
            'Gain responsibility and stability',
          ],
        },
        {
          stage: 'Year 3–5',
          bullets: [
            'Transition into supervision or safety roles',
            'Lead small teams',
          ],
        },
        {
          stage: '5+ Years',
          bullets: ['Leadership positions', 'Overseas opportunities'],
        },
      ],
      final_guidance:
    ...
    const data = await Promise.all(
      sessions.map(async (session) => {
        ...
        return {
          id: session.id,
          studentName:
            `${session.studentDetails?.personal_details?.first_name || ''} ${session.studentDetails?.personal_details?.last_name || ''}`.trim() ||
            'Unknown',
          email: session.email,
          ...
          personalityTraitName: personalityTrait
            ? personalityTrait.blendedStyleName
            : null,
          hasReport:
            session.reportData && Object.keys(session.reportData).length > 0,
