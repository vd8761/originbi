...
export class AssessmentAttempt {
    ...
    @Column({ name: 'total_score', type: 'numeric', precision: 10, scale: 2, nullable: true })
    totalScore: string | null;

    @Column({ name: 'max_score_snapshot', type: 'int', nullable: true })
    maxScoreSnapshot: number | null;

    @Column({ name: 'sincerity_index', type: 'numeric', precision: 5, scale: 2, nullable: true })
    sincerityIndex: string | null;

    @Column({ name: 'sincerity_class', type: 'varchar', length: 20, nullable: true })
