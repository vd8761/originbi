...
export class AssessmentGenerationService {
  ...
  async generateQuestions(attempt: AssessmentAttempt, manager: EntityManager) {
    ...
    if (isLevel1) {
      ...
      // Interleave 2 Main : 1 Open until BOTH pools are exhausted.
      // Driven by pool sizes (not a hard-coded 20) so the open-question
      // count stays admin-configurable while every main question is used.
