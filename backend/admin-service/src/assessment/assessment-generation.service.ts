...
export class AssessmentGenerationService {
    ...
    async generateLevel1Questions(
        ...
        this.logger.log(`Generating Level 1 questions for attempt ${attempt.id}, program ${attempt.programId}`);

        // 1. Fetch Main Questions
        const mainQuestions = await manager.createQueryBuilder(AssessmentQuestion, 'q')
        ...
        const openQuestions = await manager.createQueryBuilder(OpenQuestion, 'oq')
            ...
            for (let j = 0; j < 2; j++) {
                if (mainIdx < mainQuestions.length) {
                    ...
                    answersToInsert.push({
                        ...
                        questionOptionsOrder: JSON.stringify(this.shuffleOptions([1, 2, 3, 4])), // Store as string
                        status: 'NOT_ANSWERED'
                    });
                }
            }
        ...
        this.logger.log(`Generated ${answersToInsert.length} answers for attempt ${attempt.id}`);
    }
